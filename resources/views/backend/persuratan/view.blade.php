...
@section('content')
    ...
    <div class="card table-responsive admin-module-panel">
        <div class="card-header admin-module-header">
            <div>
                <span class="admin-module-kicker"><i class="fa-solid fa-envelope-open-text"></i> Administrasi</span>
                <h3 class="admin-module-title">{{ $title }}</h3>
                <p class="admin-module-subtitle">Lihat jenis permohonan persuratan, file masuk, file balasan, dan status proses dalam layout yang lebih bersih.</p>
            </div>
            @if (in_array(request()->user()->role, [3, 1]))
            <a href="/persuratan/add" type="button" class="btn btn-primary admin-module-cta">Ajukan</a>
            @endif
        </div>
