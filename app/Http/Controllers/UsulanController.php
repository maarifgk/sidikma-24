...
class UsulanController extends Controller
{
    ...
    public function updateInfo(Request $request)
    {
        ...
        $request->validate([
            'link_permohonan' => ['nullable', 'url', 'regex:/^https:\/\/(drive|docs)\.google\.com\//i'],
            'link_pernyataan' => ['nullable', 'url', 'regex:/^https:\/\/(drive|docs)\.google\.com\//i'],
        ], [
            'link_permohonan.regex' => 'Gunakan link file dari Google Drive.',
            'link_pernyataan.regex' => 'Gunakan link file dari Google Drive.',
        ]);
        try {
            $id = $request->id ?? 1;
            $payload = [
